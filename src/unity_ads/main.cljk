(ns unity-ads.main
  "Kotodama WASM entrypoint for the Unity_ads clean-room actor (L4) — Clojure port.
  ...
  facts (`unity_ads.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "unity_ads")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"unity_ads.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "unity_ads-compat" :tier tier :entities entities} 200])
...
(defn start! [] :unity_ads-compat/ready)
